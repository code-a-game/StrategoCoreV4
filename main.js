...
function drawGameStats() {
    const statsX = GAME_AREA_RIGHT + 50;
    const statsY = 20;
    const lineHeight = 20;
...
function mousePressed() {
    ...
    if (shared.gameState != "IN-GAME" || me.bullets.length > 2) return
...
function handleBulletMovement() {
    ...
    for (let i = me.bullets.length - 1; i >= 0; i--) {
        ...
        bullet.xLocal += bulletVector.x * parseInt(BULLET_SPEED);
        bullet.yLocal += bulletVector.y * parseInt(BULLET_SPEED);

        // Update global coordinates
        bullet.xGlobal += bulletVector.x * parseInt(BULLET_SPEED);
        bullet.yGlobal += bulletVector.y * parseInt(BULLET_SPEED);
...
function detectCollisionsAndInitiateBattles() {
    ...
    for (let i = 0; i < activeCharacters.length; i++) {
        ...
        if (char1.status === 'available') {
            ...
            console.log(`HOST: ${player1.playerName} (${char1.instanceId}) has been hit ${numberOfHits} times`);
            if (numberOfHits >= 10) {
                const char1Def = CHARACTER_DEFINITIONS.find(c => c.id === char1.id);

                if (char1Def.isCoreCommand) {
                    // ...existing code... Core Command loss handling ...
                    return;
                }
...
function resolveBattles() {
    ...
    for (const charInBattle of charactersInBattle) {
        ...
        if (opponentChar && opponentChar.status === 'inBattle' && opponentChar.inBattleWithInstanceId === charInBattle.instanceId) {
            ...
            // Always clear opponent's battle fields
...
function checkCharactersNumberOfLives() {
    let charactersNotInBattle = shared.characterList.filter(c => c.status === 'available' && c.takenByPlayerId !== null && !c.isPermanentlyLost);

    for (const charNotInBattle of charactersNotInBattle) {

        if (numberOfTimesBeingHit[charNotInBattle.takenByPlayerId] >= 10) {

            charNotInBattle.battleOutcomeResult = 'You lost because you got hit by too many bullets!'
            charNotInBattle.status = 'noMoreLives';
            charNotInBattle.isPermanentlyLost = true;
            charNotInBattle.takenByPlayerId = null;
            charNotInBattle.takenByPlayerName = null;

            // Clear battle fields
            charNotInBattle.inBattleWithInstanceId = null;
            charNotInBattle.battleOutcomeResult = null;
            charNotInBattle.battleOpponentInfo = null;
            charNotInBattle.battleStartTime = null;
        }
    }
}
